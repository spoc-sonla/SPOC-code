...
# VIDEO_QUALITY: 1=tot nhat, 51=te nhat (ap dung cho ca CPU va GPU, script tu quy doi)
VIDEO_QUALITY = 20
AUDIO_BITRATE = "192k"
...
def get_framerate(filepath):
    """Doc framerate thuc cua file, tra ve float (mac dinh 30.0 neu loi)."""
    result = subprocess.run(
...
def check_encoder_available(encoder_name):
    """Kiem tra 1 encoder ffmpeg co dung duoc tren may nay khong (test nhanh 1 frame)."""
    result = subprocess.run(
...
def get_encoder_for_codec(codec_choice):
    """
    Dua theo CODEC ("H264"/"H265"), do lan luot cac phuong an tang toc phan cung:
    NVIDIA NVENC -> Intel Quick Sync -> AMD AMF -> cuoi cung fallback ve CPU.
    Tra ve (encoder_name, label_de_hien_thi, encoder_type).
    encoder_type dung de biet cach dat co chat luong (-crf / -cq / -global_quality / -qp).
    """
...
def get_quality_flags(encoder_type, quality):
    """Moi loai encoder dung co chat luong khac nhau, quy doi tu thang VIDEO_QUALITY (1=tot, 51=te)."""
...
def main():
    ...
    # Doc framerate goc de giu nguyen sau khi tang toc
    src_fps = get_framerate(files[0])
    # Sau khi tang toc bang setpts, so frame/giay giu nguyen nhung thoi gian ngan lai
    # -> output fps = src_fps (ffmpeg tu tinh, nhung mot so encoder GPU can explicit -r)
    out_fps = round(src_fps)  # mot so encoder GPU chi chap nhan fps nguyen
    print(f"      FPS nguon: {src_fps:.3f} -> output fps: {out_fps}")
